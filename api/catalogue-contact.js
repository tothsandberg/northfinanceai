...
export default async function handler(req, res) {
  ...
  // ─── Verify the code: KV first, then env-var fallback (legacy/test codes) ───
  let customer = null;
  let source = null;

  try {
    customer = await kvGet(`catalogue:active:${code}`);
    if (customer) source = 'kv';
  } catch (e) {
  ...
  // Defensive fallbacks: legacy/test records may lack email
  const customerEmail = (customer.email || '').trim();
  const hasCustomerEmail = customerEmail.length > 0 && customerEmail.includes('@');
  ...
  try {
    await sendEmail({
      to: OWNER_EMAIL,
      replyTo: hasCustomerEmail ? customerEmail : OWNER_EMAIL,
      subject: source === 'env-var'
        ? `🧪 [TEST] Detail-Call angefragt — ${customer.name}${customer.company ? ' (' + customer.company + ')' : ''}`
        : `🎯 Detail-Call angefragt — ${customer.name}${customer.company ? ' (' + customer.company + ')' : ''}`,
      html: `
        <div style="font-family: Georgia, 'Times New Roman', serif; max-width: 600px; color: #2a2a2a; line-height: 1.6;">
          <div style="border-top: 3px solid #b8935a; padding-top: 24px;">
            <div style="font-size: 11px; letter-spacing: 0.2em; text-transform: uppercase; color: ${source === 'env-var' ? '#888' : '#b8935a'}; font-weight: 700; margin-bottom: 8px;">
              ${source === 'env-var' ? 'Testmodus · Env-Var Code' : 'Heißer Lead'}
            </div>
          ...
          <div style="background: #f7f3ec; padding: 20px 24px; margin: 24px 0; border-left: 3px solid #b8935a;">
            <table cellpadding="6" style="border-collapse: collapse; font-size: 14px;">
              ...
              <tr><td style="color: #777;"><strong>E-Mail:</strong></td><td>${hasCustomerEmail ? `<a href="mailto:${esc(customerEmail)}" style="color: #b8935a; text-decoration: none;">${esc(customerEmail)}</a>` : '<em style="color: #999;">— nicht im Datensatz —</em>'}</td></tr>
          ...
          ${source === 'env-var' ? `
          <div style="background: #fff8ec; border: 1px solid #e8d8b8; padding: 16px 20px; margin: 24px 0; font-size: 13px; color: #6b5530;">
            <strong>Hinweis:</strong> Diese Anfrage kam über einen <strong>Test-/Legacy-Code</strong> aus der CATALOGUE_CODES Env-Var. Im Datensatz fehlt die E-Mail-Adresse — daher kann der Kunde nicht automatisch zurückbestätigt werden. In echten Produktionsfällen (Code aus KV) ist immer eine E-Mail hinterlegt.
          </div>
          ` : `
          ...
          <p style="font-size: 13px; color: #777; margin-top: 32px;">
            Auf <strong>„Antworten"</strong> klicken schreibt direkt an <strong>${esc(customerEmail)}</strong>.
          </p>
          ` : ''}
    ...
    return res.status(500).json({ error: 'Failed to send notification', detail: String(err && err.message || err) });
  }

  // ─── Send confirmation to customer (only if we have their email) ───
  if (hasCustomerEmail) {
    try {
      await sendEmail({
        to: customerEmail,
  ...
  return res.status(200).json({ ok: true, testMode: source === 'env-var' });
}
